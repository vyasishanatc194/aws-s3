<?php
function wpawss3_view_completed_reports_files() {
    if ( is_admin()){
        return;
    }
?>
...
<script>
    jQuery(document).ready( function($) {
        ...
        var table = $('#faq_table').DataTable( {
            "processing": true,
            ajax: {
                url: pw1_script_vars.ajaxurl + '?action=view_completed_files_list&security='+pw1_script_vars.security
            },
            columns: [
                { data: 'filename',},
                { data: 'filetypeLabel' },
...
add_shortcode('wpawss3viewcompletedreportsfiles', 'wpawss3_view_completed_reports_files');
?>