...
add_action('wp_ajax_nopriv_view_process_status_list', 'view_process_status_list');



function view_error_process_list() {
    check_ajax_referer('wpawss3', 'security');
    $result['success'] = false;
    $result['data'] = [
        'message' => 'Network error.',
    ...
    try {
		...
		$allowed_roles = array('administrator');
		if( array_intersect($allowed_roles, $user->roles ) ) {  
		   $sql_stmt = "SELECT * FROM `view_prs_app_process` WHERE StatusLabel = 'Error'";
		}else{
			$sql_stmt = "SELECT * FROM `view_prs_app_process` WHERE StatusLabel = 'Error' AND idUser = $userId";
		} 
...
add_action('wp_ajax_view_error_process_list', 'view_error_process_list');
add_action('wp_ajax_nopriv_view_error_process_list', 'view_error_process_list');



function view_error_files_list() {
    check_ajax_referer('wpawss3', 'security');
    $result['success'] = false;
    $result['data'] = [
        'message' => 'Network error.',
...
function view_completed_files_list() {
    ...
    try {
		...
		if( array_intersect($allowed_roles, $user->roles ) ) {  
		   ...
		   $sql_stmt = "SELECT * FROM `view_prs_files` WHERE StatusLabel = 'Complete' ";
			
		}else{
			$sql_stmt = "SELECT * FROM `view_prs_files` WHERE StatusLabel = 'Complete' AND idUser = $userId";
